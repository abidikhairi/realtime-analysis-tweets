...
from utils import prepare_sequence, emotion_labels, load_or_build_vocab
	...
	for epoch in range(num_epochs):
		...
		with torch.no_grad():
			...
			if test_accuracy > 0.50:
				conf_matrix = metrics.confusion_matrix(torch.tensor(test_preds).squeeze(1), torch.tensor(test_labels), num_classes=num_classes)
				cmd = ConfusionMatrixDisplay(conf_matrix.numpy(), display_labels=emotion_labels.keys())
				cmd.plot()
				plt.savefig(f'./data/emotion-confusion-matrix-{epoch + 1}.png')
				plt.close()
				torch.save(model.state_dict(), f'./data/lstm-emotion-{test_accuracy*100:.2f}.pt')
